...
create or replace function add_prodprof(p_nsn char, p_slc_num int, p_inv_station_no_fk char, p_cost_center_no_fk int, p_generic_name text,
  ...
  p_model text, p_remark text, p_description text,	p_model text, p_amount numeric)
    returns text as
$$
  declare
    v_asset_code int;
    ...
    select asset_code, cost_center_no_fk from product into v_asset_code, v_cost_center_no
     ...
     if v_asset_code isnull and v_cost_center_no isnull then
      insert into product(nsn, slc_num, inv_station_no_fk, cost_center_no_fk, generic_name, brand, part_num, manufacture_date, 
            ...
            item_name, serial_number, model, remark, description, model, amount)
            ...
            p_item_name, p_serial_number, p_model, p_remark, p_description,  p_model, p_amount);
      return 'Successfully added';
    else
      update product
      
      set item_name = p_item_name, amount = p_amount, nsn = p_nsn, slc_num = p_slc_num, 
        inv_station_no = p_inv_station_no_fk, cost_center_no_fk = p_cost_center_no_fk, generic_name = p_generic_name, brand = p_brand,
        part_num = p_part_num, manufacture_date = p_manufacture_date, expiry_date = p_expiry_date, class = p_class, stock = p_stock, 
        block = p_block, unit_measure = p_unit_measure, unit_cost = p_unit_cost, quantity = p_quantity, average_amt = p_average_amt,
